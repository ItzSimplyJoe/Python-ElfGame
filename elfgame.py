...
class ElfGame:
    ...
    def daycalculator(self,daynumber,elftotal,money):
        ...
        if daynumber == 25:
            ...
            sg.popup("You can now purchase new elves for £75 per elf!")
            sg.popup(f"Today is day {daynumber}, You have {elftotal} elves left, and £{money}")
            self.first17days(money,elftotal,daynumber,False,False)
        elif daynumber > 20:
            sg.popup(f"Today is day {daynumber}, You have {elftotal} elves left, and £{money}")
            self.first17days(money,elftotal,daynumber,False,False)
        elif daynumber == 18:
            sg.popup("A New path has been built to allow you to access the \n Far Far Away Forest. \n Each tree here is worth £50 \nhowever if there is a blizzard you're elves will DIE!!!!")
            sg.popup(f"Today is day {daynumber}, You have {elftotal} elves left, and £{money}")
    ...
    def Strike(self,daynumber,elftotal,money):
        ...
        window = sg.Window("Elf Game", layout)
        while True:
            event,values = window.read()
            if event == sg.WINDOW_CLOSED:
                break
        ...
        if dayOff != True:
            ...
            if choice == 1:
                sg.popup("The Elves have chosen to strike")
                sg.popup("Its now two days later and your elves have returned to work")
    ...
    def taxMan(self,money): 
        ...
        if payTax != True:
            choice = random.randint(1,2)
            if choice == 1:
                sg.popup("The tax man is back and you must now pay your taxes")
                moneydeduct = money/5
                sg.popup(f"You have been taxed 20% of your money \n which is £{moneydeduct}")
                money = money - (money/5)
            else:
                sg.popup("You avoided the tax man. Lucky You!")
        else:
            ...
            if event == sg.WINDOW_CLOSED:
                ...
                if lottNum.isdigit() == False:
                    ...
                    if lottNum == successNum:
                        window.close()
                        moneywon = 500
                        sg.popup("You won the lottery, you have won $500")
                        return moneywon
                    else:
                        window.close()
                        sg.popup(f"You did not win the lottery, the number was {successNum}")
                        moneywon = 0
                        return moneywon
